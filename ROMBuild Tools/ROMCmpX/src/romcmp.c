...
enum {	MODE_A,
		...
		MODE_E14, MODE_O14, MODE_E24, MODE_O24,
		MODE_E34, MODE_O34, MODE_E44, MODE_O44,
		TOTAL_MODES };
char *modenames[] =
{
	"          ",
...
static void compatiblemodes(int mode,int *start,int *end)
{
	...
	if (mode >= MODE_E14 && mode <= MODE_O44)
	{
		*start = MODE_E14;
		*end = MODE_O44;
	}
...
static void basemult(const struct fileinfo *file,int mode,int *base,int *mult)
{
	*mult = 1;
	...
	switch (mode)
	{
		...
		case MODE_O24:
			*base = 1 + file->size / 4; break;
		case MODE_34:
		case MODE_E34:
			*base = 2*file->size / 4; break;
		case MODE_O34:
			*base = 1 + 2*file->size / 4; break;
		case MODE_44:
		case MODE_E44:
			*base = 3*file->size / 4; break;
		case MODE_O44:
			*base = 1 + 3*file->size / 4; break;
	}
}
...
static void printname(const struct fileinfo *file1,const struct fileinfo *file2,float score,int mode1,int mode2)
{
	...
	*/
	...
	if ((score==1 && mode1!=MODE_A) || (score==1 && mode2!=MODE_A))
	{
		printf("\t{\n");

		if (mode2==MODE_A || mode2==MODE_12 || mode2==MODE_22 || mode2==MODE_14 || mode2==MODE_24 || mode2==MODE_34 || mode2==MODE_44) printf("\t\tFULL_COPY,\n");
		if (mode2==MODE_E || mode2==MODE_E12 || mode2==MODE_E22 || mode2==MODE_E14 || mode2==MODE_E24 || mode2==MODE_E34 || mode2==MODE_E44) printf("\t\tEVEN_COPY,\n");
		if (mode2==MODE_O || mode2==MODE_O12 || mode2==MODE_O22 || mode2==MODE_O14 || mode2==MODE_O24 || mode2==MODE_O34 || mode2==MODE_O44) printf("\t\tODD_COPY,\n");

		if (mode1==MODE_A) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, 0, file1->size);

		if (mode1==MODE_12) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, 0, file1->size/2);

		if (mode1==MODE_22) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, file1->size/2, file1->size/2);

		if (mode1==MODE_14) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, 0, file1->size/4);

		if (mode1==MODE_24) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, file1->size/4, file1->size/4);

		if (mode1==MODE_34) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, file1->size/4*2, file1->size/4);

		if (mode1==MODE_44) printf("\t\t{\"game1\", \"%s\", %d, 0x%08lx, %d, %d},\n", file1->name, file1->size, file1->crc32, file1->size/4*3, file1->size/4);

		if (mode2==MODE_A) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size);

		if (mode2==MODE_12) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/2);

		if (mode2==MODE_22) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/2, file2->size/2);

		if (mode2==MODE_14) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/4);

		if (mode2==MODE_24) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/4, file2->size/4);

		if (mode2==MODE_34) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/4*2, file2->size/4);

		if (mode2==MODE_44) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/4*3, file2->size/4);

		if (mode2==MODE_E) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/2);

		if (mode2==MODE_O) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/2);

		if (mode2==MODE_E12) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/4);

		if (mode2==MODE_O12) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/4);

		if (mode2==MODE_E22) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/2, file2->size/4);

		if (mode2==MODE_O22) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/2, file2->size/4);

		if (mode2==MODE_E14) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/8);

		if (mode2==MODE_O14) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 0, file2->size/8);

		if (mode2==MODE_E24) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/4, file2->size/8);

		if (mode2==MODE_O24) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, file2->size/4, file2->size/8);

		if (mode2==MODE_E34) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 2*file2->size/4, file2->size/8);

		if (mode2==MODE_O34) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 2*file2->size/4, file2->size/8);

		if (mode2==MODE_E44) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 3*file2->size/4, file2->size/8);

		if (mode2==MODE_O44) printf("\t\t{\"game2\", \"%s\", %d, 0x%08lx, %d, %d},\n", file2->name, file2->size, file2->crc32, 3*file2->size/4, file2->size/8);
			...
			while((d = readdir(dir)) != NULL)
			{
				...
				if(stat(buf, &st_file) == 0 && S_ISREG(st_file.st_mode))
				{
					...
					else
					{
						...
						files[i][found[i]].crc32 = crc32(0, NULL, 0);
						files[i][found[i]].crc32 = crc32(files[i][found[i]].crc32, files[i][found[i]].buf, st_file.st_size);
